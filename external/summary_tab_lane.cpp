 ...
 */
template<typename I>
void print_array(std::ostream& out, I beg, I end)
{
    for(;beg != end;++beg)
    {
        out << std::left << *beg;
 ...
 */
template<size_t N>
void print_array(std::ostream& out, const char*(&values)[N])
{
    print_array(out, values, values+N);
}
/** Take a array of strings and print them using a fixed width
 *
 * @param out output stream
 ...
 */
template<size_t N>
void print_array(std::ostream& out, std::string(&values)[N])
{
    print_array(out, values, values+N);
}
 ...
 */
void print_array(std::ostream& out, const std::vector<std::string>& values)
{
    print_array(out, values.begin(), values.end());
}
...
void summarize(const lane_summary& summary, std::vector<std::string>& values)
{
    ...
    values[0] = util::lexical_cast<std::string>(summary.tile_count());
    values[1] = format(summary.density(), 0, 0, 1e3);
    values[2] = format(summary.density_pf(), 0, 0, 1e3);
    values[3] = format(summary.percent_pf(), 0, 2);
    values[4] = format(summary.cluster_count(), 0, 2);
    values[5] = format(summary.cluster_count_pf(), 0, 2);
    values[6] = util::format(summary.phasing().mean(), 3, 3) + " / " + util::format(summary.prephasing().mean(), 3, 3);
    values[7] = format(summary.reads(), 0, 0);
    values[8] = format(summary.reads_pf(), 0, 0);
    values[9] = format(summary.percent_gt_q30(), 0, 2);
    values[10] = format(summary.yield_g(), 0, 2);
    values[11] = format(summary.cycle_state().error_cycle_range());
    values[12] = format(summary.percent_aligned(), 0, 2);
    values[13] = format(summary.error_rate(), 0, 2);
    values[14] = format(summary.error_rate_35(), 0, 2);
    values[15] = format(summary.error_rate_75(), 0, 2);
    values[16] = format(summary.error_rate_100(), 0, 2);
    values[17] = format(summary.first_cycle_intensity(), 0, 0);
}
...
void print_summary(std::ostream& out, const run_summary& summary)
{
    ...
    print_array(out, values);
    out<<"#Per Lane Statistics\n";

    const char* lane_header[] = {"Lane", "Read", "Tiles", "Density(k/mm2)", "Density PF", "Cluster PF%", 
				 "Cluster Counts", "Cluster Counts PF", "Phase/Prephase", "Reads",
                                 "Reads PF", "%>=Q30", "Yield", "Cycles Error", "Aligned%", "Error%",
                                 "Error (35)%", "Error (75)%", "Error (100)%", "Intensity C1" };
    values.resize(size_of(lane_header));
    print_array(out, lane_header);
    for(size_t lane=0;lane<summary.lane_count();++lane)
    {
        for(size_t read=0;read<summary.size();++read)
        {
            ...
            out << util::lexical_cast<std::string>(lane+1) << '\t';
            out << format_read(summary[read].read()) << '\t';
            print_array(out, values);
        }
    }
}
